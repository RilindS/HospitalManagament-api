...
const CreateVacationFormForNurse = ({ onCreateComplete }) => {
    ...
    const decodeToken = () => {
      ...
      if (token) {
        try {
          ...
          setNurseId(decodedToken?.userId || ""); // Use userId as doctorId
        } catch (error) {
  ...
  const onFinish = async (values) => {
    try {
      const payload = {
        ...values,
        nurseId, // Include doctorId from userId in token
      };
...
export default CreateVacationFormForNurse;
